...
#
# Wires together the seven linters in this repo so CI (or a pre-commit hook) can
# call ONE thing instead of seven:
#
...
# checks fail on style choices, not bugs, so they're opt-in. The four ON-by-default
# checks only fail on genuine defects (a dead link, an unclosed fence, a missing
# alt attribute, malformed frontmatter), so they're safe to gate every build.
#
